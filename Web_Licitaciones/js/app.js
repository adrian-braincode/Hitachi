...
const App = {
  ...
  _bindUpload() {
    ...
    input.addEventListener('change', e => { if (e.target.files[0]) this._loadFile(e.target.files[0]); input.value = ''; });

    document.getElementById('pmaxEco').addEventListener('input', () => this._updatePmaxHint());
    document.getElementById('pmaxTec').addEventListener('input', () => this._updatePmaxHint());
  },
  ...
  async _loadFile(file) {
    ...
    status.className = 'upload-status show upload-status--loading';
    status.textContent = '⏳ Leyendo archivo…';
    try {
      const data = await this._parseExcel(file);
      if (data.presupuesto) document.getElementById('presupuesto').value = data.presupuesto.toLocaleString('es-ES', { maximumFractionDigits: 2 });
      if (data.pmaxEco) {
        document.getElementById('pmaxEco').value = data.pmaxEco;
        document.getElementById('pmaxTec').value = Math.max(0, 100 - data.pmaxEco).toFixed(0);
      }
      document.getElementById('empresasBody').innerHTML = '';
      data.empresas.forEach(e => this._addRow(e));
      if (!data.empresas.length) { this._addRow(); this._addRow(); }
      status.className = 'upload-status show upload-status--success';
      status.textContent = `✓ ${file.name} — Hoja: "${data.sheetName}" — ${data.empresas.length} empresas`;
      this._updatePmaxHint();
    } catch (err) {
      status.className = 'upload-status show upload-status--error';
  ...
  _parseExcel(file) {
    return new Promise((resolve, reject) => {
      if (typeof XLSX === 'undefined') { reject(new Error('Librería XLSX no disponible (requiere conexión)')); return; }
      const reader = new FileReader();
      reader.onerror = () => reject(new Error('Error de lectura'));
      reader.onload = e => {
        try {
          const wb = XLSX.read(e.target.result, { type: 'array' });
          const PREFERRED = ['PUNTUACION TOTAL (5 comp)', 'PUNTUACION TOTAL (4 comp)', 'PUNT. TOTAL (CALC.)'];
          let sheetName = wb.SheetNames[0], sheet = wb.Sheets[sheetName];
          for (const n of PREFERRED) { if (wb.SheetNames.includes(n)) { sheetName = n; sheet = wb.Sheets[n]; break; } }
          const cell = ref => { const c = sheet[ref]; return c ? c.v : null; };
          const presupuesto = cell('B2');
          const pmaxEco     = cell('B45') ?? cell('B7');
          const empresas = [];
          for (let r = 51; r <= 85; r++) {
            const nombre = cell(`D${r}`), bajaRaw = cell(`A${r}`), valoracion = cell(`B${r}`), experiencia = cell(`J${r}`) ?? 0;
            if (bajaRaw == null && nombre == null) continue;
            let baja = null;
            if (typeof bajaRaw === 'number') baja = bajaRaw > 0 && bajaRaw < 1 ? bajaRaw * 100 : bajaRaw;
            if (baja && baja > 0 && baja < 100) {
              const nombreStr = nombre ? String(nombre).trim() : '';
              empresas.push({ nombre: nombreStr, baja, valoracionTecnica: typeof valoracion === 'number' ? valoracion : 0, experiencia: typeof experiencia === 'number' ? experiencia : 0, esHitachi: /hitachi/i.test(nombreStr), estado: 'valida' });
            }
          }
          resolve({ presupuesto, pmaxEco, empresas, sheetName });
